...
export default function DealerLeadQueryScreen() {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      if (!editingLead || !editForm) throw new Error("No data to update");
    ...
    onError: (err: any) => {
      setSaveError(err?.message || "Failed to update — check with TL.");
    },
  });
      ...
      <View style={styles.header}>
        <View style={styles.titleRow}>
          ...
          <TouchableOpacity style={styles.createBtn} onPress={() => router.push("/dealer-lead-query-create")}>
            <Feather name="plus" size={14} color={colors.white} /><Text style={styles.createBtnText}>Create</Text>
          </TouchableOpacity>
        </View>
